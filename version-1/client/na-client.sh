...
FRPC_CONF1="$SCRIPT_DIR/frpc1.toml"
FRPC_CONF2="$SCRIPT_DIR/frpc2.toml"
FRPC_CONF="$FRPC_CONF1" # Default for read ops
PM2_NAME1="frpc1"
PM2_NAME2="frpc2"
PM2_NAME="frpc1"
...
test_connection() {
    for NODE in 1 2; do
        echo -e "
[0;36m>>> Đang kiểm tra kết nối đến Server $NODE...[0m"
        CONF_FILE=$(eval echo "\$FRPC_CONF$NODE")
        
        if [ ! -f "$CONF_FILE" ]; then
             echo -e "[0;31m❌ Chưa có thông tin cấu hình.[0m"
             continue
        fi
        SERVER_IP=$(grep -E "^serverAddr" "$CONF_FILE" | cut -d'"' -f2)
        SERVER_PORT=$(grep -E "^serverPort" "$CONF_FILE" | awk '{print $3}')
        
        if [[ -z "$SERVER_IP" || -z "$SERVER_PORT" ]]; then
            echo -e "[0;31m❌ Chưa có thông tin IP/Port.[0m"
            continue
        fi
        echo -e "Kiểm tra → IP: [0;33m$SERVER_IP[0m | Port: [0;33m$SERVER_PORT[0m"
        if nc -z -w 4 "$SERVER_IP" "$SERVER_PORT" 2>/dev/null || bash -c "cat < /dev/null > /dev/tcp/$SERVER_IP/$SERVER_PORT" 2>/dev/null; then
            echo -e "[0;32m✔ Kết nối TCP thành công![0m"
        else
            echo -e "[0;31m❌ Không thể kết nối đến Server $NODE![0m"
        fi
    done
}
    ...
    while true; do
        ...
        if grep -q "localPort = $port" "$FRPC_CONF" && grep -q "type = \"http\"" "$FRPC_CONF"; then
            if [[ "$OSTYPE" == "darwin"* ]]; then
                sed -i '' "/localPort = $port/,/customDomains/ s/\]/,\"$domain\"\]/" "$FRPC_CONF1"

    sed -i '' "/localPort = $port/,/customDomains/ s/\]/,\"$domain\"\]/" "$FRPC_CONF2"
            else
                sed -i "/localPort = $port/,/customDomains/ s/\]/,\"$domain\"\]/" "$FRPC_CONF1"
...
delete_domain() {
    while true; do
        ...
        if [[ "$input" =~ ^[0-9]+$ ]]; then
            ...
            # Xóa domain HTTP
            if [[ "$OSTYPE" == "darwin"* ]]; then
                sed -i '' "s/\"$input\",//g; s/,\"$input\"//g; s/\"$input\"//g" "$FRPC_CONF1"

    sed -i '' "s/\"$input\",//g; s/,\"$input\"//g; s/\"$input\"//g" "$FRPC_CONF2"
            else
                sed -i "s/\"$input\",//g; s/,\"$input\"//g; s/\"$input\"//g" "$FRPC_CONF1"
...
change_port() {
    while true; do
        ...
        if [[ "$OSTYPE" == "darwin"* ]]; then
            sed -i '' "s/\"$domain\",//g; s/,\"$domain\"//g; s/\"$domain\"//g" "$FRPC_CONF1"

    sed -i '' "s/\"$domain\",//g; s/,\"$domain\"//g; s/\"$domain\"//g" "$FRPC_CONF2"
        else
            sed -i "s/\"$domain\",//g; s/,\"$domain\"//g; s/\"$domain\"//g" "$FRPC_CONF1"
    ...
    sed -i "s/\"$domain\",//g; s/,\"$domain\"//g; s/\"$domain\"//g" "$FRPC_CONF2"
        ...
        if grep -q "localPort = $new_port" "$FRPC_CONF"; then
            if [[ "$OSTYPE" == "darwin"* ]]; then
                sed -i '' "/localPort = $new_port/,/customDomains/ s/\]/,\"$domain\"\]/" "$FRPC_CONF1"

    sed -i '' "/localPort = $new_port/,/customDomains/ s/\]/,\"$domain\"\]/" "$FRPC_CONF2"
            else
                sed -i "/localPort = $new_port/,/customDomains/ s/\]/,\"$domain\"\]/" "$FRPC_CONF1"
...
config_server() {
    echo -e "
[0;36m--- CẤU HÌNH KẾT NỐI SERVER (DUAL HA) ---[0m"
    
    for NODE in 1 2; do
        echo -e "
[0;33m>>> CẤU HÌNH SERVER $NODE <<<[0m"
        CONF_FILE=$(eval echo "\$FRPC_CONF$NODE")
        PM2_INST=$(eval echo "\$PM2_NAME$NODE")
        
        cur_ip=$(grep -E "^serverAddr" "$CONF_FILE" 2>/dev/null | cut -d'"' -f2)
        cur_port=$(grep -E "^serverPort" "$CONF_FILE" 2>/dev/null | awk '{print $3}')
        cur_token=$(grep -E "^auth\.token" "$CONF_FILE" 2>/dev/null | cut -d'"' -f2)
        cur_api_port=$(grep -E "^# API_PORT=" "$CONF_FILE" 2>/dev/null | cut -d'=' -f2)
        cur_api_token=$(grep -E "^# API_TOKEN=" "$CONF_FILE" 2>/dev/null | cut -d'=' -f2)
        
        read -p "Server $NODE IP [$cur_ip]: " ip
        ip=${ip:-$cur_ip}
        ...
        sed -n '/\[\[proxies\]\]/,$p' "$CONF_FILE" > /tmp/frpc_proxies_$NODE.tmp 2>/dev/null || true
        
        # WebServer port: 7400 for Node 1, 7401 for Node 2
        WPORT=$((7400 + NODE - 1))
...
port = $WPORT
EOL
        if [ -s /tmp/frpc_proxies_$NODE.tmp ]; then
            echo "" >> "$CONF_FILE"
            cat /tmp/frpc_proxies_$NODE.tmp >> "$CONF_FILE"
        fi
        rm -f /tmp/frpc_proxies_$NODE.tmp
        
        pm2 restart "$PM2_INST" >/dev/null 2>&1 || true
    done
    test_connection
}
...
run_frpc() {
    if ! command -v frpc >/dev/null 2>&1; then
        echo -e "[0;31m❌ Chưa cài đặt frpc. Vui lòng chọn mục 3.[0m"
        return
    fi
    for NODE in 1 2; do
        CONF_FILE=$(eval echo "\$FRPC_CONF$NODE")
        PM2_INST=$(eval echo "\$PM2_NAME$NODE")
        echo -e "[0;33mĐang khởi động FRPC Node $NODE...[0m"
        
        # Check if conf exists
        if [ ! -f "$CONF_FILE" ]; then
            echo -e "[0;31m❌ Chưa cấu hình Server $NODE.[0m"
            continue
        fi
        
        pm2 delete "$PM2_INST" >/dev/null 2>&1
        pm2 start "$(which frpc)" --name "$PM2_INST" -- -c "$CONF_FILE" >/dev/null 2>&1
    done
    pm2 save >/dev/null 2>&1
    echo -e "[0;32m✔ Đã khởi động 2 tiến trình FRPC![0m"
}
